...
from .spin_velocity import calculate_spin_velocities_jit
...
class LLGS_Simulation_2D:
    ...
    def __init__(
        ...
        self.H_DMI = (
            np.zeros((3, self.N, self.N)) if H_DMI is None else np.asarray(H_DMI)
        )
        if self.H_DMI.shape != (3, self.N, self.N):
            raise ValueError(
                f"H_DMI must have shape (3, {self.N}, {self.N}), "
                f"got {self.H_DMI.shape}"
            )
